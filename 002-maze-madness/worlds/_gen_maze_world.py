...
def maze_geometry(num, name, moves, z_offset):
    ...
    return floor_set, wall_set, redstones, spawn, diamond, len(path), pillar
...
def build_py(geoms):
    ...
    body += "def clear_zone():\n"
    body += "    blocks.fill(AIR, pos(2, -4, -8), pos(30, 10, 650), FillOperation.REPLACE)\n"
    body += "    blocks.fill(GRASS, pos(2, -5, -8), pos(30, -5, 650), FillOperation.REPLACE)\n\n\n"
...
- **Start pillar**: 4-block `GLOWSTONE` tower above each start cell (visible from far away)
...
Mazes build **10 blocks east** of player position (X_OFFSET = 10) so the build
does not engulf the player. Stand still, type `build1`, then walk/teleport
agent to the start pillar.
